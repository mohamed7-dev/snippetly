 ...
 */
export function createApiErrorGuard<T>(testFn: (input: T) => boolean): ApiErrorGuard<T> {
    return new ApiErrorGuard<T>(testFn);
}
...
export class ApiErrorGuard<T> {
     ...
     */
    assertApiError<R>(input: T | R): asserts input is R {
        if (this.isSuccess(input)) {
