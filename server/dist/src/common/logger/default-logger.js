 ...
 */ export class DefaultLogger {
    ...
    rotateLogFileIfNeeded() {
        ...
        if (DefaultLogger.currentLogDate !== today) {
            ...
            const logDir = process.env.LOG_PATH || path.resolve(process.cwd(), "logs");
    ...
    error(message, context, trace, meta) {
        if (this.level >= LogLevel.Error) {
            const stringifiedMessage = this.ensureString(message + (trace ? `\n${trace}` : ""));
    ...
    getTimestampLog() {
        const timestamp = new Date(Date.now()).toLocaleString(undefined, this.localeTimestampOptions);
