import { boolean, check, index, integer, pgEnum, pgTable, text, timestamp, unique, uniqueIndex } from "drizzle-orm/pg-core";
import { baseTable } from "./utils.js";
...
export const usersRelations = relations(usersTable, ({ many })=>({
        ...
        friendshipsRequested: many(friendshipsTable, {
            relationName: "requester"
        }),
        friendshipsReceived: many(friendshipsTable, {
...
export const friendshipsTable = pgTable("friendships", {
    ...
    addresseeId: integer("addressee_id").references(()=>usersTable.id, {
        onDelete: "cascade"
    }).notNull(),
...
export const collectionsTable = pgTable("collections", {
    ...
    forkedFrom: integer("forked_from").references(()=>collectionsTable.id, {
        onDelete: "set null"
    }),
    creatorId: integer("creator_id").references(()=>usersTable.id, {
        ...
        index("collection_title_search_index").using("gin", sql`to_tsvector('english', ${t.title})`),
        index("collection_description_search_index").using("gin", sql`to_tsvector('english', ${t.description})`)
    ]);
export const collectionsRelations = relations(collectionsTable, ({ one, many })=>({
        creator: one(usersTable, {
            fields: [
                collectionsTable.creatorId
            ],
            references: [
                usersTable.id
            ]
...
export const snippetsTable = pgTable("snippets", {
    ...
    isPrivate: boolean("is_private").default(false).notNull(),
    allowForking: boolean("allow_forking").default(true).notNull(),
    forkedFrom: integer("forked_from").references(()=>snippetsTable.id, {
        onDelete: "set null"
    }),
    creatorId: integer("creator_id").references(()=>usersTable.id, {
        onDelete: "cascade"
    ...
    collectionId: integer("collection_id").references(()=>collectionsTable.id, {
        ...
        index().on(t.oldSlugs),
        uniqueIndex().on(t.slug),
        index("snippet_title_search_index").using("gin", sql`to_tsvector('english', ${t.title})`),
        index("snippet_description_search_index").using("gin", sql`to_tsvector('english', ${t.description})`)
    ]);
export const snippetsRelations = relations(snippetsTable, ({ one, many })=>({
        creator: one(usersTable, {
            fields: [
                snippetsTable.creatorId
            ],
            references: [
                usersTable.id
            ]
...
export const collectionsTagsRelations = relations(collectionsTagsTable, ({ one })=>({
        collection: one(collectionsTable, {
            fields: [
                collectionsTagsTable.collectionId
            ],
            references: [
                collectionsTable.id
            ]
...
export const snippetsTagsRelations = relations(snippetsTagsTable, ({ one })=>({
        snippet: one(snippetsTable, {
            fields: [
                snippetsTagsTable.snippetId
            ],
            references: [
                snippetsTable.id
            ]
