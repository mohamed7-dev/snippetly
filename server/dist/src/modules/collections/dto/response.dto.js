...
export const DiscoverCollectionsResDto = z.array(SelectCollectionDto.omit({
    isPrivate: true,
    id: true,
    creatorId: true,
    forkedFrom: true,
    ...
    const { slug, creator: { name, ...creatorRes }, createdAt, ...rest } = val;
    return {
        ...rest,
        addedAt: createdAt,
        publicId: slug,
...
const UserCollectionsSchema = z.object({
    ...
    collections: z.array(SelectCollectionDto.omit({
        id: true,
        creatorId: true,
        oldSlugs: true
    }).extend({
        ...
        snippets: z.array(SelectSnippetDto.pick({
            title: true,
            slug: true,
            language: true,
            createdAt: true
...
export const GetCurrentUserCollectionsResDto = z.object({
    ...
    collections: z.array(UserCollectionsSchema.shape.collections.unwrap().transform((val)=>{
        const { creator: { name, ...creatorRest }, snippets, createdAt, slug, updatedAt, forkedFrom, ...rest } = val;
        return {
            ...rest,
            publicId: slug,
            addedAt: createdAt,
...
export const GetPublicUserCollectionsResDto = z.object({
    stats: UserCollectionsSchema.shape.stats,
    collections: z.array(UserCollectionsSchema.shape.collections.unwrap().omit({
        forkedFrom: true,
        updatedAt: true,
        isPrivate: true
    }).transform((val)=>{
        const { creator: { name, ...creatorRest }, snippets, createdAt, slug, ...rest } = val;
        return {
            ...rest,
            publicId: slug,
            addedAt: createdAt,
...
export const GetCollectionResDto = UserCollectionsSchema.shape.collections.unwrap().extend({
    snippetsCount: z.number()
}).transform((val)=>{
    const { creator: { name, ...creatorRest }, snippets, createdAt, slug, updatedAt, forkedFrom, ...rest } = val;
    return {
...
export const GetPublicCollectionResDto = UserCollectionsSchema.shape.collections.unwrap().omit({
    forkedFrom: true,
    updatedAt: true,
    isPrivate: true
}).extend({
