...
import { CLIENTS_URLS, JWT_REFRESH_EXPIRES, JWT_REFRESH_REMEMBER_EXPIRES } from '../../config/index';
...
export class AuthService {
    ...
    public async login(ctx: RequestContext, input: LoginRequestDtoType, res: Response) {
        ...
        if (!foundUser) throw new HttpException(StatusCodes.UNAUTHORIZED, 'Invalid credentials.');
    ...
    public async signup(ctx: RequestContext, input: SignupRequestDtoType, res: Response) {
        try {
    ...
    public async sendVerificationEmail(_ctx: RequestContext, input: SendVEmailRequestDtoType, user?: User) {
        const { email } = input;
        const { token, user: foundUser } = await this.TokenService.generateEmailVerificationToken(
            email,
            user,
        );
    ...
    public async verifyVerificationToken(_ctx: RequestContext, input: VerifyVTokenRequestDtoType) {
        const token = input.token;
        const user = await this.TokenService.verifyEmailVerificationToken(token);
        return { user, token };
    }

    public async sendResetPasswordEmail(_ctx: RequestContext, input: SendRTokenRequestDtoType) {
        const { email } = input;
    ...
    public async resetPassword(
        ctx: RequestContext,
        input: VerifyRTokenRequestQueryDtoType & VerifyRTokenRequestBodyDtoType,
    ) {
        const token = input.token;
        const foundUser = await this.TokenService.verifyResetPasswordToken(token);
        const updatedUser = await this.UserService.updatePassword(ctx, {
    ...
    private setRefreshTokenCookie(res: Response, refreshToken: string, rememberMe: boolean) {
