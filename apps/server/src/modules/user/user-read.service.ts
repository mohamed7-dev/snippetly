import { and, arrayContains, desc, eq, inArray, like, lt, not, or, sql } from 'drizzle-orm';
import { DiscoverUsersRequestQueryDtoType } from '../../../../../packages/common/dist/schema';
...
export class UserReadService {
     ...
     * without joining.
     */
    public async findOneSlim(by: 'name' | 'id' | 'email', value: string | number, exact?: boolean) {
        const foundUser = await Database.client.query.usersTable.findFirst({
            where: (t, { or, eq, like }) => {
                const conditions = [];

                if (by === 'name') {
                    conditions.push(exact ? eq(t.name, value as string) : like(t.name, value as string));
                }
     ...
     * without joining.
     */
    async findManySlim(by: 'name' | 'id' | 'email', value: string | number, exact?: boolean) {
                ...
                and(
                    ...
                    cursor
                        ? or(
                              ...
                              and(eq(snippetsCount, cursor.snippetsCount), lt(usersTable.id, cursor.id)),
                          )
                        : undefined,
    ...
    async getUserForDashboard({ userId }: { userId: number }) {
        ...
        const countsMap = Object.fromEntries(snippetsCounts.map(c => [c.collectionId, Number(c.count)]));
    ...
    `,
        ...
        const [[snippetStats], [collectionStats], [friendStats]] = await Promise.all([
            snippetQuery,
            collectionQuery,
            friendsQuery,
        ]);
    ...
    > {
        ...
        if (!loggedInUserId) {
            return await Database.client.query.usersTable.findFirst({
                where: (t, { eq, and }) =>
                    and(eq(t.id, id), isCurrentUserOwner ? undefined : eq(t.isPrivate, false)),
                columns: {
                    password: false,
                },
            });
