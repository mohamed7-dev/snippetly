...
import { GetCurrentUserFriendsRequestQueryDtoType } from '../../../../../packages/common/dist/schema';
import { Database } from '../../common/db/index';
import { type Friendship, friendshipsTable, snippetsTable, usersTable } from '../../common/db/schema';
    ...
    async getUserInbox({
        ...
        const friendsInboxCountQuery = Database.client
            ...
            .innerJoin(
                ...
                and(eq(usersTable.id, fi.requesterId), eq(fi.addresseeId, userId), eq(fi.status, 'pending')),
            )
            .where(
                searchString
                    ? or(
        ...
        const [data, total] = await Promise.all([findFriendsInboxQuery, friendsInboxCountQuery]);
    ...
    async getUserOutbox({
        ...
        const friendsOutboxCountQuery = Database.client
            ...
            .innerJoin(
                ...
                and(eq(usersTable.id, fi.addresseeId), eq(fi.requesterId, userId), eq(fi.status, 'pending')),
            )
            .where(
                searchString
                    ? or(
        ...
        const [data, total] = await Promise.all([findFriendsOutboxQuery, friendsOutboxCountQuery]);
        return { data, total: total[0]?.count ?? 0 };
    }
}
