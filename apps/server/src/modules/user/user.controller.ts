...
export class UserController {
    ...
    public update = async (
        req: Request<object, object, Omit<UpdateUserRequestDtoType, 'image' | 'imageKey'>>,
        res: Response,
    ) => {
        ...
        const { success, data: parsedData } = UpdateUserResponseDto.safeParse(rawResponse);
    ...
    public delete = async (req: Request, res: Response) => {
        ...
        const { success, data: parsedData } = DeleteUserResponseDto.safeParse(rawResponse);
    ...
    public discoverUsers = async (
        ...
        const result = await this.UserService.discoverUsers(req.context, req.validatedQuery);
        ...
        const { success, data: parsedData } = DiscoverUsersResponseDto.safeParse(rawResponse);
    ...
    public getCurrentUserDashboard = async (req: Request, res: Response) => {
        ...
        const { success, data: parsedData } = GetCurrentUserDashboardResDto.safeParse(rawResponse);
    ...
    public getCurrentUserProfile = async (req: Request, res: Response) => {
        ...
        if ('profile' in result && result.profile) {
            ...
            const { success, data: parsedData } = GetCurrentUserResponseDto.safeParse(rawResponse);
            if (!success) {
                throw new InternalServerError();
            }
            res.status(parsedData.status).json(parsedData);
    ...
    public getUserProfile = async (req: Request<GetUserRequestDtoType>, res: Response) => {
