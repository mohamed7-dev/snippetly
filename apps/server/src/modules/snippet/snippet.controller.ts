...
export class SnippetController {
    ...
    public create = async (req: Request<object, object, CreateSnippetRequestDtoType>, res: Response) => {
    ...
    public update = async (
        req: Request<UpdateSnippetRequestParamDtoType, object, UpdateSnippetRequestBodyDtoType>,
        res: Response,
    ) => {
        ...
        if ('redirect' in result) {
            ...
            const rawRes = {
                ...
                message: `Snippet has been updated successfully${
                    req.body.collectionSlug && !collectionId ? ", but the collection couldn't be found." : '.'
                }`,
                data: updatedSnippet,
            } satisfies UpdateSnippetResDtoType['success'];
            const { success, data: parsedData } = UpdateSnippetResDto.safeParse(rawRes);
    ...
    public fork = async (
        req: Request<ForkSnippetRequestParamDtoType, object, ForkSnippetRequestBodyDtoType>,
        res: Response,
    ) => {
        ...
        if ('redirect' in result) {
            res.redirect(StatusCodes.TEMPORARY_REDIRECT, `/snippets/${result?.slug}/fork`);
        } else {
            const rawRes = {
                type: 'success',
                status: StatusCodes.OK,
    ...
    public delete = async (req: Request<DeleteSnippetRequestParamDtoType>, res: Response) => {
        ...
        if ('redirect' in result) {
            ...
            const { success, data: parsedData } = DeleteSnippetResDto.safeParse(rawRes);
            if (!success) {
                throw new InternalServerError();
            }
            res.status(parsedData.status).json(parsedData);
    ...
    public getSnippetsByCollection = async (
        ...
        if ('redirect' in result) {
            ...
            const isCurrentUserOwner = req.context.user.id === result.collection.creatorId;
            const rawRes = {
            ...
            const { success, data: parsedData } = GetCollectionSnippetsResDto.safeParse(rawRes);
            if (!success) {
                throw new InternalServerError();
            }
    ...
    public discover = async (
        ...
        const result = await this.SnippetService.discover(req.context, req.validatedQuery);
        const rawRes = {
            type: 'success',
            message: 'Fetched successfully.',
            status: StatusCodes.OK,
        ...
        const { success, data: parsedData } = DiscoverSnippetsResDto.safeParse(rawRes);
        if (!success) {
            throw new InternalServerError();
        }
        res.status(parsedData.status).json(parsedData);
    ...
    public getCurrentUserSnippets = async (
        ...
        const result = await this.SnippetService.getCurrentUserSnippets(req.context, {
            ...req.validatedQuery,
        });
        if (!('redirect' in result)) {
            const rawRes = {
            ...
            const { success, data: parsedData } = GetUserSnippetsResDto.safeParse(rawRes);
            if (!success) {
                throw new InternalServerError();
            }
            res.status(parsedData.status).json(parsedData);
    ...
    public getUserSnippets = async (
        req: Request<GetUserSnippetsRequestParamDtoType, object, object, GetUserSnippetsRequestQueryDtoType>,
        res: Response,
    ) => {
    ...
    public getCurrentUserFriendsSnippets = async (
        ...
        const result = await this.SnippetService.getCurrentUserFriendsSnippets(req.context, {
            ...req.validatedQuery,
        });
        ...
        const { success, data: parsedData } = GetUserFriendsSnippetsResDto.safeParse(rawRes);
    ...
    public getSnippet = async (req: Request<GetSnippetRequestParamDtoType>, res: Response) => {
