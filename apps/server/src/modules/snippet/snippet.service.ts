...
import { generateUniquePrefix, handleCursorPagination, slugify } from '../../common/lib/utils';
...
import { DISCOVER_SNIPPETS_DEFAULT_LIMIT, FIND_SNIPPETS_DEFAULT_LIMIT } from './constants';
...
export class SnippetService {
    ...
    public async create(
        ...
        let foundCollection = await this.CollectionReadService.findOneSlim('slug', collectionSlug);
        if (!foundCollection) {
            foundCollection = await this.CollectionReadService.findOneSlimByOldSlug(collectionSlug);
        }

        if (!foundCollection) {
            throw new HttpException(StatusCodes.NOT_FOUND, `Collection ${collectionSlug} is not found.`);
        }
    ...
    public async update(
        ...
        if (collectionSlug) {
            const foundCollection = await this.CollectionReadService.findOneSlim('slug', collectionSlug);
            if (!foundCollection) {
    ...
    public async delete(ctx: NonNullableFields<RequestContext>, input: DeleteSnippetRequestParamDtoType) {
        const { slug } = input;
        const userId = ctx.user.id;
    ...
    public async discover(ctx: RequestContext, input: DiscoverSnippetsRequestQueryDtoType) {
        const { limit } = input;
    ...
    public async getUserSnippets(
        ctx: RequestContext,
        input: Partial<GetUserSnippetsRequestQueryDtoType & GetUserSnippetsRequestParamDtoType> & {
            user?: User;
        },
    ) {
    ...
    public async findOne(ctx: RequestContext, input: GetSnippetRequestParamDtoType) {
        const foundSnippet = await this.SnippetsReadService.findOneSlim('slug', input.slug);
        if (!foundSnippet) {
    ...
    public async getSnippetsByCollection(
        ctx: RequestContext,
        input: GetCollectionSnippetsRequestParamDtoType & GetCollectionSnippetsRequestQueryDtoType,
    ) {
        const defaultLimit = input.limit ?? FIND_SNIPPETS_DEFAULT_LIMIT;
        const foundCollection = await this.CollectionReadService.findOneSlim('slug', input.collectionSlug);

        if (!foundCollection) {
            const foundCollectionWithOldSlug = await this.CollectionReadService.findOneSlimByOldSlug(
        ...
        const { data, total } = await this.SnippetsReadService.findSnippetsByCollection(
            {
                ...input,
                limit: defaultLimit,
            },
