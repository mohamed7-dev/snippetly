    ...
    async discover({
        ...
        const [data, total] = await Promise.all([
          ...
          `.as('tags'),
                ...
                .innerJoin(collectionsTable, eq(collectionsTable.id, snippetsTable.collectionId))
                .where(
                    and(
                        ...
                        query
                            ...
                            : undefined,
                        loggedInUserId ? not(eq(snippetsTable.creatorId, loggedInUserId)) : undefined,
                    ),
                )
                .limit(limit + 1)
    ...
    async findUserSnippets(
        {
            ...
            query,
            cursor,
        }: GetUserSnippetsRequestQueryDtoType & Required<Pick<GetUserSnippetsRequestQueryDtoType, 'limit'>>,
        creatorId: number,
        isCurrentUserOwner: boolean,
    ) {
        const [data, total] = await Promise.all([
