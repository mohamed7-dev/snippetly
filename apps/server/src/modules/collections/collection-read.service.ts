import { and, arrayContains, desc, eq, getTableColumns, lt, not, or, sql } from 'drizzle-orm';
import {
          ...
          `.as('snippetsCount'),
                ...
                .where(
                    and(
                        ...
                        cursor ? lt(collectionsTable.updatedAt, cursor.updatedAt) : undefined,
                        query
                            ? or(
                            ...
                            : undefined,
                        loggedInUserId ? not(eq(collectionsTable.creatorId, loggedInUserId)) : undefined,
                    ),
                )
                .limit(limit + 1)
        ...
        const [data, snippetsCounts, total] = await Promise.all([
            ...
            Database.client.$count(
                ...
                and(
                    ...
                    isCurrentUserOwner ? undefined : eq(collectionsTable.isPrivate, false),
                    query
                        ? or(
                              sql`to_tsvector('english', ${collectionsTable.title}) @@ plainto_tsquery('english', ${query})`,
                              sql`to_tsvector('english', ${collectionsTable.description}) @@ plainto_tsquery('english', ${query})`,
        ...
        const [stats] = await Database.client
            .select({
                ...
                publicCollections: Database.client.$count(
                    collectionsTable,
                    and(eq(collectionsTable.creatorId, userId), eq(collectionsTable.isPrivate, false)),
                ),
