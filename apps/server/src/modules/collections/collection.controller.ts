...
export class CollectionController {
    ...
    public create = async (
        ...
        const result = await this.CollectionService.create(request.context, request.body);
        const rawResponse = {
            type: 'success',
        ...
        const { success, data: parsedData } = CreateCollectionResponseDto.safeParse(rawResponse);
    ...
    public update = async (
        request: Request<UpdateCollectionRequestParamDtoType, object, UpdateCollectionRequestBodyDtoType>,
        response: Response,
    ) => {
        ...
        if ('redirect' in result) {
            ...
            const { success, data: parsedData } = UpdateCollectionResDto.safeParse(rawResponse);
            if (!success) {
                throw new InternalServerError();
            }
            response.status(parsedData.status).json(parsedData);
    ...
    public fork = async (request: Request<ForkCollectionRequestParamDtoType>, response: Response) => {
        ...
        if ('redirect' in result) {
            ...
            const { success, data: parsedData } = ForkCollectionResDto.safeParse(rawResponse);
            if (!success) {
                throw new InternalServerError();
            }
            response.status(parsedData.status).json(parsedData);
    ...
    public delete = async (request: Request<DeleteCollectionRequestParamDtoType>, response: Response) => {
        ...
        if ('redirect' in result) {
            ...
            const { success, data: parsedData } = DeleteCollectionResDto.safeParse(rawResponse);
    ...
    public discover = async (
        req: Request<object, object, object, DiscoverCollectionsRequestQueryDtoType>,
        res: Response,
    ) => {
        const result = await this.CollectionService.discover(req.context, req.validatedQuery);
        const rawResponse = {
            type: 'success',
        ...
        const { success, data: parsedData } = DiscoverCollectionsResDto.safeParse(rawResponse);
    ...
    public getCurrentUserCollections = async (
        ...
        const { success, data: parsedData } = GetUserCollectionsResDto.safeParse(rawResponse);
    ...
    public getUserCollections = async (
        ...
        if ('redirect' in result) {
            ...
            const isCurrentUserOwner = request.context?.user?.id === result?.items?.[0]?.creator?.id;
    ...
    public getCollection = async (request: Request<GetCollectionRequestParamDtoType>, response: Response) => {
        const result = await this.CollectionService.findOne(request.context, request.params);
        ...
        if ('redirect' in result) {
            ...
            const isCurrentUserOwner = request.context?.user?.id === (result as Collection)?.creatorId;
            ...
            const { success, data: parsedData } = GetCollectionResDto.safeParse(rawResponse);
            if (!success) {
                throw new InternalServerError();
            }
